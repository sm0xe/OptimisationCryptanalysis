...
using custom_sga_selection_map_t = boost::bimap<std::string, custom_sga_selection>;
using custom_sga_crossover_map_t = boost::bimap<std::string, custom_sga_crossover>;
using custom_sga_mutation_map_t = boost::bimap<std::string, custom_sga_mutation>;

// Helper init functions
custom_sga_selection_map_t custom_sga_init_selection_map()
{
    custom_sga_selection_map_t retval;
    using value_type = custom_sga_selection_map_t::value_type;
    retval.insert(value_type("tournament", custom_sga_selection::TOURNAMENT));
    retval.insert(value_type("truncated", custom_sga_selection::TRUNCATED));
    return retval;
}

custom_sga_crossover_map_t custom_sga_init_crossover_map()
{
    custom_sga_crossover_map_t retval;
    using value_type = custom_sga_crossover_map_t::value_type;
    retval.insert(value_type("exponential", custom_sga_crossover::EXPONENTIAL));
    retval.insert(value_type("binomial", custom_sga_crossover::BINOMIAL));
    //retval.insert(value_type("sbx", sga_crossover::SBX));
    retval.insert(value_type("single", custom_sga_crossover::SINGLE));
    return retval;
}

custom_sga_mutation_map_t custom_sga_init_mutation_map()
{
    custom_sga_mutation_map_t retval;
    using value_type = custom_sga_mutation_map_t::value_type;
    retval.insert(value_type("gaussian", custom_sga_mutation::GAUSSIAN));
    retval.insert(value_type("uniform", custom_sga_mutation::UNIFORM));
    retval.insert(value_type("polynomial", custom_sga_mutation::POLYNOMIAL));
    retval.insert(value_type("index_swap", custom_sga_mutation::INDEX_SWAP));
    return retval;
}

const custom_sga_selection_map_t custom_sga_selection_map = custom_sga_init_selection_map();
const custom_sga_crossover_map_t custom_sga_crossover_map = custom_sga_init_crossover_map();
const custom_sga_mutation_map_t custom_sga_mutation_map = custom_sga_init_mutation_map();

} // namespace

} // namespace detail

custom_sga::custom_sga(unsigned gen, double cr, double eta_c, double m, double param_m, unsigned param_s, std::string crossover,
         std::string mutation, std::string selection, bool uniqueness, bool playfair, unsigned seed)
    : m_gen(gen), m_cr(cr), m_eta_c(eta_c), m_m(m), m_param_m(param_m), m_param_s(param_s), m_e(seed), m_seed(seed),
      m_verbosity(0u), m_log(), m_uniqueness(uniqueness), m_playfair(playfair)
{
    if (cr > 1. || cr < 0.) {
    ...
    if (mutation != "gaussian" && mutation != "uniform" && mutation != "polynomial" && mutation != "index_swap") {
        pagmo_throw(
            std::invalid_argument,
            R"(The mutation type must either be "gaussian" or "uniform" or "polynomial" or "index_swap": unknown type requested: )"
                + mutation);
    }
    ...
    if (crossover != "exponential" && crossover != "binomial" && crossover != "single") {
        pagmo_throw(
            std::invalid_argument,
            R"(The crossover type must either be "exponential" or "binomial" or "single": unknown type requested: )"
                + crossover);
    }
    ...
    m_crossover = detail::custom_sga_crossover_map.left.at(crossover);
    m_mutation = detail::custom_sga_mutation_map.left.at(mutation);
    m_selection = detail::custom_sga_selection_map.left.at(selection);
}
 ...
 */
population custom_sga::evolve(population pop) const
{
    ...
    if (m_crossover == detail::custom_sga_crossover::SBX && (pop.size() % 2 != 0u)) {
        pagmo_throw(std::invalid_argument,
 ...
 */
void custom_sga::set_seed(unsigned seed)
{
    m_e.seed(seed);
    m_seed = seed;
}
 ...
 */
std::string custom_sga::get_extra_info() const
{
    std::ostringstream ss;
    stream(ss, "\tNumber of generations: ", m_gen);
    stream(ss, "\n\tCrossover:");
    stream(ss, "\n\t\tType: " + detail::custom_sga_crossover_map.right.at(m_crossover));
    stream(ss, "\n\t\tProbability: ", m_cr);
    if (m_crossover == detail::custom_sga_crossover::SBX) stream(ss, "\n\t\tDistribution index: ", m_eta_c);
    stream(ss, "\n\tMutation:");
    stream(ss, "\n\t\tType: ", detail::custom_sga_mutation_map.right.at(m_mutation));
    stream(ss, "\n\t\tProbability: ", m_m);
    if (m_mutation != detail::custom_sga_mutation::POLYNOMIAL) {
        stream(ss, "\n\t\tWidth: ", m_param_m);
    } else {
        stream(ss, "\n\t\tDistribution index: ", m_param_m);
    }
    stream(ss, "\n\tSelection:");
    stream(ss, "\n\t\tType: ", detail::custom_sga_selection_map.right.at(m_selection));
    if (m_selection == detail::custom_sga_selection::TRUNCATED) stream(ss, "\n\t\tTruncation size: ", m_param_s);
    if (m_selection == detail::custom_sga_selection::TOURNAMENT) stream(ss, "\n\t\tTournament size: ", m_param_s);
    stream(ss, "\n\tSeed: ", m_seed);
...
void custom_sga::serialize(Archive &ar, unsigned)
{
...
std::vector<vector_double::size_type> custom_sga::perform_selection(const std::vector<vector_double> &F) const
{
    ...
    std::iota(best_idxs.begin(), best_idxs.end(), vector_double::size_type(0u));
    switch (m_selection) {
        case (detail::custom_sga_selection::TRUNCATED): {
        ...
        case (detail::custom_sga_selection::TOURNAMENT): {
...
void custom_sga::perform_crossover(std::vector<vector_double> &X, const std::pair<vector_double, vector_double> &bounds,
                            vector_double::size_type dim_i) const
{
    ...
    // selected couple.
    if (m_crossover == detail::custom_sga_crossover::SBX) {
      /*
        assert(X.size() % 2u == 0u);
        std::shuffle(X.begin(), X.end(), m_e);
        for (decltype(X.size()) i = 0u; i < X.size(); i += 2) {
            auto children = detail::sbx_crossover_impl(X[i], X[i + 1], bounds, dim_i, m_cr, m_eta_c, m_e);
            X[i] = children.first;
        ...
        */
        ...
        for (decltype(X.size()) i = 0u; i < X.size(); ++i) {
            ...
            switch (m_crossover) {
                case (detail::custom_sga_crossover::EXPONENTIAL): {
                ...
                case (detail::custom_sga_crossover::BINOMIAL): {
                    auto n = rnd_gene_idx(m_e);
                ...
                case (detail::custom_sga_crossover::SINGLE): {
                    if (drng(m_e) < m_cr) {
                        auto n = rnd_gene_idx(m_e);
            ...
            // Ensure unique elements in chromosome
            if(m_uniqueness){
              int used[26] = {0};
              if(m_playfair) used[9]=1;
              for(int j=0; j<dim; j++){
                if(used[int(round(child[j]))]==0){
                  used[int(round(child[j]))]=1;
                }
                else{
                  child[j]=-1;
                }
...
void custom_sga::perform_mutation(std::vector<vector_double> &X, const std::pair<vector_double, vector_double> &bounds,
                           vector_double::size_type dimi) const
{
    ...
    for (decltype(X.size()) i = 0u; i < X.size(); ++i) {
        ...
        switch (m_mutation) {
            case (detail::custom_sga_mutation::UNIFORM): {
            ...
            case (detail::custom_sga_mutation::GAUSSIAN): {
                // Start of main loop through the chromosome
                for (decltype(N) j = 0u; j < N; ++j) {
                    auto gene_idx = to_be_mutated[j];
            ...
            case (detail::custom_sga_mutation::POLYNOMIAL): { // https://www.iitk.ac.in/kangal/papers/k2012016.pdf
                // Start of main loop through the chromosome
                for (decltype(N) j = 0u; j < N; ++j) {
                    auto gene_idx = to_be_mutated[j];
                    if (gene_idx < dimc) {
            ...
            case (detail::custom_sga_mutation::INDEX_SWAP): {
               vector_double individual = X[i];
               rnd_lb_ub.param(std::uniform_int_distribution<int>::param_type(0,dim-1));
               auto first = rnd_lb_ub(m_e);
               auto second = rnd_lb_ub(m_e);
               X[i][first] = individual[second];
               X[i][second] = individual[first];
               break;
            }
        }
        ...
        if(m_uniqueness){
          ...
          if(m_playfair) used[9]=1;
          for(int j=0; j<dim; j++){
            if(used[int(round(X[i][j]))]==0){
              used[int(round(X[i][j]))]=1;
            }
            else{
              X[i][j]=-1;
            }
