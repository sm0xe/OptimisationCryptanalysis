...
def fitness(point):
    ...
    return abs(pow(point[1],2)-pow(point[0],2))%n
...
def spiral_optimization(kmax=100,points=5):
    ...
    #r = pow(0.5,0.25)
    #kstar = 0
    point_list = np.random.random((points,2))*n//1
    ...
    #lin = np.linspace(1,n//2,points)//1
    ...
    for i in range(kmax):
        if i%50==0:
            np.round(point_list)
            np.round(best_point)
            best_fitness = fitness(best_point)
        ...
        if min(fitness_list)<best_fitness:
            best_fitness = min(fitness_list)
            where = np.argmin(fitness_list)
            best_point = point_list[where]
            if best_fitness==0.0: return best_point
        print(best_point)
        print(best_fitness)
        """
